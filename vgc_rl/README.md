...
Default counts: **30** slot actions (= 4×7 move/target combos + **2** switches), **898** joint pairs (= 30² minus **two** pairs where **both** slots switch into the **same** bench index). Most pairs are still **illegal** for real battles until you add rules and **action masking** ([Action masking tutorial](https://gymnasium.farama.org/tutorials/training_agents/action_masking_taxi_env/)).
...
Use this environment when you want **one Gymnasium step = one full doubles turn**: your policy picks Alpha’s **898-way structural joint** (`enumerate_joint_actions_structural` in [`vgc_rl/doubles_actions.py`](vgc_rl/doubles_actions.py)); Beta samples a **uniform random legal joint** under the same masking rules (swap Beta for your SB3 policy when you integrate masking at inference). Switch and move phases call the HTTP oracle (`speedCompare` ordering, then `single` damage rows). Training runs need a **live oracle** (`oracle-server`); unit tests swap in a **`FakeOracleClient`** that returns deterministic batch rows.
